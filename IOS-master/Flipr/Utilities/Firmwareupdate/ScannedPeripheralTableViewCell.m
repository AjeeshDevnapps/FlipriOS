...
//    peripheralAdressLabel.text  = @"Click to upgrade";
...
//    borderView.alpha = 0.5;
    ...
    NSString *name = @"";
    name = [self nameForPeripheral:discoveredPeripheral];
    NSString *rssVal = @"";
    rssVal = [self RSSIValueForPeripheral:discoveredPeripheral];
    
    NSString *rssValstr = @"(";
    [rssValstr stringByAppendingString:@"RSSI "];
    [rssValstr stringByAppendingString:rssVal];
    [rssValstr stringByAppendingString:@")"];


    NSString *upgradeStr = LOCALIZEDSTRING(@"Upgrade");
    if (self.isDiagnostic == TRUE){
        upgradeStr = @"Diagnostic";
    }else{
        upgradeStr = @"Upgrade";
    }
    
    NSString *titleStr = [NSString stringWithFormat: @"%@ %@ ",upgradeStr, name];


//    [name appendString:[self nameForPeripheral:discoveredPeripheral]];
    peripheralName.text  =  [NSString stringWithFormat: @"%@", titleStr];
    [peripheralName setFont:[UIFont boldSystemFontOfSize:15]];
    // CONFIGURATORS-2444
